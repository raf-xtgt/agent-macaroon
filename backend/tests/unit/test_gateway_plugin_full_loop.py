...
async def test_full_delegation_and_enforcement_loop() -> None:
    ...
    registry.register(
        ...
        max_scope={"read", "fetch"},  # "delete" excluded
        owner="research-team",
    )
    registry.register(
        agent_id="tool_caller_agent",
        display_name="Tool Caller Leaf Agent",
        max_scope={"read"},  # "fetch" and "delete" excluded
    ...
    assert current_scope(token_step1) == frozenset({"read", "delete", "fetch"})
    ...
    assert current_scope(token_step2) == frozenset({"read", "delete", "fetch"})
    ...
    # Narrowed from {"read", "delete", "fetch"} to {"read", "fetch"}
    assert current_scope(token_step3) == frozenset({"read", "fetch"})
    ...
    # Narrowed from {"read", "fetch"} to {"read"}
